...
mu.1 = rep(0, p); mu.2 = rep(0, p); d3 = 1;
  ...
  for(d2 in c(10, 50, 100)){
    for( a  in c(3,4) ){
      for(b in c(4,5 )){
        ...
        X <- c(rep(0,p-(d1 + d2 + d3)),rep(1,d1),rep(2,d2),rep(3,d3));
        mu.1[X==1] = rnorm(d1, a, 1); mu.1[X==3] = 4
        mu.2[X==2] = rnorm(d2, b, 1); mu.2[X==3] = 4
        p.vec = list()
        p.vec.2 = list()
        p.vec.3 = list()
        for(i in 1:600){
          ...
          for(thresh in c( 10^(-2), 10^(-3), 10^(-4), 10^(-5) )){
            temp = c(temp, any((P1 < thresh) & (P2 < thresh)) )
          }
          p.vec.3[[i]] = temp
        }
        print(mean(p.vec <= 0.05));
        print(mean(p.vec.2 <= 0.05));
        res = rbind(res, c(d1, d2, a, b,
                           mean(p.vec <= 0.05),
                           mean(p.vec.2 <= 0.05),
                           colMeans(do.call(rbind, p.vec.3) )) )
